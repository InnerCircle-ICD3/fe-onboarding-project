...
describe("useTotalPrice 테스트", () => {
  ...
  it("increaseTotalPrice()에 전달된 값만큼 totalPrice 값이 증가한다.", () => {
    const { result } = renderHook(() => useTotalPrice());
  ...
  it("increaseTotalPrice()에 숫자값이 아닌 문자열이 전달되면 totalPrice 값이 증가하지 않고 오류가 발생한다.", () => {
    const { result } = renderHook(() => useTotalPrice());
  ...
  it("decreaseTotalPrice()에 전달된 값만큼 totalPrice 값이 감소한다.", () => {
    const { result } = renderHook(() => useTotalPrice());
  ...
  it("decreaseTotalPrice()에 숫자값이 아닌 문자열이 전달되면 totalPrice 값이 감소하지 않고 오류가 발생한다.", () => {
    const { result } = renderHook(() => useTotalPrice());
    const prevTotalPrice = result.current.totalPrice;
  ...
  it("increaseTotalPrice()로 증가한 값이 resetTotalPrice()로 초기화된다.", () => {
    const { result } = renderHook(() => useTotalPrice());
